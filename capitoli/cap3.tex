...
\begin{tabular}{| c | c | c |}
 \hline
    ...
    \hline
\end{tabular}
\caption{Esempio di mapping con riconfigurazione necessaria.}
...
trasferimento su BUS se un altro trasferimento tiene occupata la risorsa.
...
Al momento di effettuare lo scheduling di una comunicazione, si possono 
...
BUS.
\end{enumerate}
Nel primo caso, la comunicazione viene banalmente assegnata all'esecuzione su 
...
Nel secondo caso, dev'essere controllato se la comunicazione è ``eleggibile'' 
...
della memoria si occupa di cercare una porzione di memoria libera 
sufficientemente grande per contenere i dati prodotti dal task. Se non viene 
...
Ogni volta che una comunicazione che ha come sorgente il processing task per 
cui è stata allocata memoria viene schedulata, il corrispondente successore 
...
patto che la dimensione sia compatibile con il numero di dati da memorizzare.
