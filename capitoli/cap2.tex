...
anche in un altro loro lavoro \cite{BanerjeeReconfigurationOverhead}
...
\subparagraph{Modello ILP per architetture riconfigurabili a una dimensione}
...
\subparagraph{Modello ILP per architetture riconfigurabili a due dimensioni}
...
\cite{Redaelli2DILP} propongono una formulazione ILP adatta ad architetture con 
riconfigurazione parziale dinamica a due dimensioni.
...
Alcune variabili del modello ILP proposte in \cite{Redaelli1DILP} sono 
...
\paragraph{Limitazioni formulazioni ILP}
...
costo in termini di tempo d'esecuzione del \emph{solver ILP} aumenta 
esponenzialmente.
...
approssimata. Uno stesso problema può essere risolto utilizzando più di una 
euristica, ma non è garantito che tutte le euristiche applicate a uno stesso 
problema trovino la stessa soluzione, anzi, di solito questo non si verifica.

% TODO

Come rappresentato nella Figura \ref{fig:metaheuristicsClassification}, esistono 
diverse categorie di meta-euristiche, che si ispirano alle basi del 
funzionamento di sistemi presenti in natura oppure si basano su criteri che 
rispecchiano le teorie sull'evoluzione di una popolazione
