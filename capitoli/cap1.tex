...
%trattati nel corso della tesi, alle motivazioni e a una panoramica del contesto in 
...
La sezione \ref{sec:reconfComp} descrive come il progresso tecnologico e la sempre maggior
richiesta in termini di prestazioni delle applicazioni, unita alla necessit\`a di ridurre
il costo di progettazione di sistemi hardware pongano dei requisiti continuamente soggetti a
cambiamenti; l'utilizzo di architetture riconfigurabili permette di soddisfare questi
requisiti.
%contiene una descrizione dell'area generale in cui si 
%svolge il lavoro, presentando i concetti fondamentali alla base del \emph{reconfigurable 
%computing}.

La sezione \ref{sec:definizioneProblema} introduce le problematiche che si riscontrano nella
progettazione di un sistema per architetture riconfigurabili, la necessit\`a di una metodologia
di sviluppo che aiuti il designer nel difficile compito di progettare il sistema e che determini inoltre
quando l'utilizzo della riconfigurazione porta benefici alle prestazioni del sistema. Particolare
enfasi viene posta sul sotto-problema dello \emph{scheduling}, oggetto del lavoro, e all'importanza
che riveste questa fase all'interno del flusso di sviluppo introdotto in precedenza.
