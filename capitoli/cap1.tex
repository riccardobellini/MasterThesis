...
flusso di sviluppo e sul perchè questo problema sia impegnativo da risolvere
...
\subsubsection{Fasi del flusso} L'applicazione viene dapprima codificata in una
rappresentazione interna al flusso, che ne consente la manipolazione e
l'elaborazione da parte del framework. Esistono diverse possibili
rappresentazioni possibili, nel contesto di questo lavoro ci riferiremo alla
rappresentazione tramite \emph{task graph}, descritta nella sezione
\ref{subsec:mappingScheduling}. L'applicazione viene quindi suddivisa in un
...
eseguiti in hardware e quali su un processore general-purpose (fase di
\emph{mapping}), oltre ad assegnare le risorse computazionali ai vari task in
base al partizionamento calcolato; in questa fase si effettua anche lo
...
La fase di mapping, che spesso incorpora il partizionamento hardware/software, ha il
...
di ottimizzazione multiobiettivo) è un classico esempio di \ac{RCSP}; tale
